...
export class PanelLayoutController {
  ...
  private createHandle(
    ...
    if (reverse && kind === 'explorer') {
      // The explorer handle is centered on its panel's left edge (the final
      // margin is written by applyGrid); the preview handle starts AT the
      // boundary and extends into the panel, keeping the conversation
      // scrollbar clear.
  ...
  private applyGrid(): void {
    ...
    if (this.explorerHandle !== null) {
      ...
      // The hit zone extends RIGHT into the explorer panel (its visual line
      // sits at the boundary): the conversation pane's right-edge scrollbar
      // stays clear of the zone, adjacent but not overlapped.
      this.explorerHandle.style.marginLeft = '0px'
    ...
    if (this.previewHandle !== null) {
      ...
      // The hit zone extends RIGHT into the preview panel (its visual line
      // sits at the boundary): the conversation pane's right-edge scrollbar
      // stays clear of the zone, adjacent but not overlapped.
      this.previewHandle.style.marginLeft = '0px'
