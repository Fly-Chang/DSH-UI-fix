 ...
 * affinity ledger, the selected character and the persisted display config.
 * The API gateway maps this service's methods onto `pet.state` /
 * `pet.interact` / `pet.setCharacter` / `pet.setVisible` / `pet.setConfig`
...
export interface PetSettingsSection {
  ...
  bottom: number
  /** User-customizable display name of the CURRENT character. */
  name: string
  /** Master switch for the plugin (browser half + host routes). */
  enabled?: boolean
}
...
export interface PetStateView {
  ...
  /** Character roster summary (switch button + treat copy). */
  ...
  /** Current character's display name. */
  name: string
  /** Treat (投喂物) stock snapshot. */
  treats: {
    /** Stocked treats now. */
    stocked: number
    /** Stock cap. */
...
export class PetService extends Service {
  ...
  petName(): string {
    return petDisplayName(this.persist)
  }
  ...
  /** RPC: switch the pet character; shared affinity and treats stay intact. */
  async setCharacter(character: CharacterId): Promise<PetStateView> {
    if (!isCharacterId(character)) throw new Error('invalid-character')
    if (this.persist.character === character) return this.view()
    this.persist = { ...this.persist, character }
    this.flush()
  ...
  async interact(kind: PetInteraction): Promise<PetInteractResult> {
    ...
    const def = CHARACTERS[this.persist.character]
    ...
    if (!outcome.accepted) {
      return {
        reaction: kind === 'pet' ? def.reactions.petCooldown : def.reactions.feedCooldown,
        delta: 0,
    ...
    if (kind === 'feed') {
      ...
      if (!consume.ok) {
        return {
          reaction: fillReaction(def.reactions.noTreat, def),
          delta: 0,
          affinity: this.affinityView(this.persist.affinity),
        }
      }
    ...
    return {
      reaction: kind === 'pet' ? def.reactions.petOk : def.reactions.feedOk,
      delta: outcome.delta,
      affinity,
    }
  }
  ...
  /** RPC: rename the CURRENT character (trimmed, 1–20 chars). */
  async setName(name: string): Promise<{ ok: true; name: string } | { ok: false; error: string }> {
    ...
    this.persist = {
      ...this.persist,
      names: { ...this.persist.names, [this.persist.character]: trimmed },
    }
    this.flush()
    this.syncSettingsFromPet()
   ...
   * the setConfig RPC so both write paths converge. The section name applies
   * to the CURRENT character only.
  ...
  applySettingsSection(section: PetSettingsSection): void {
    ...
    this.persist = {
      ...this.persist,
      display: next,
      ...(name === '' ? {} : { names: { ...this.persist.names, [this.persist.character]: name } }),
    }
    this.flush()
  }
  ...
  private syncSettingsFromPet(): void {
    ...
    void settings.update(PET_SETTINGS_NAMESPACE, {
      ...
      name: petDisplayName(this.persist),
    }).catch(() => {
  ...
  private view(): PetStateView {
    ...
    return {
      ...
      character: this.persist.character,
      characters: Object.values(CHARACTERS).map(characterSummary),
      affinity: this.affinityView(this.persist.affinity),
  ...
  private affinityView(affinity: AffinityState): PetStateView['affinity'] {
    const nowMs = Date.now()
    const def = CHARACTERS[this.persist.character]
    const rank = rankOf(affinity.points, def.ranks)
    return {
