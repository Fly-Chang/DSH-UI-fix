 ...
 * same-origin JSON endpoints (`/api/pet/*`) and loads each character's atlas
 * from `/pet/whale/*` or `/pet/phrolova/{light,dark}/*`. The `/plugins/`
 * endpoint only serves client bundles and RPC domains are
 * platform-registered, so the pet serves its own API and media — the same
 * pattern as dsh-remote-web-ui's `/api/pair` family.
...
/** Browser-facing base path of the whale-girl asset routes (kept stable). */
export const PET_ASSET_PREFIX = '/pet/whale'

/** One character form exposed under its own prefix. */
const ASSET_GROUPS = [
  { prefix: '/pet/whale', assetDir: 'whale' },
  { prefix: '/pet/phrolova/light', assetDir: 'phrolova/light' },
  { prefix: '/pet/phrolova/dark', assetDir: 'phrolova/dark' },
] as const

/** Relative (to package root) asset files exposed under each prefix. */
const ASSET_FILES = [
...
export function makePetRoutes(deps: { service: PetService; packageRoot: string }): WebRoute[] {
  ...
  const apiRoutes: WebRoute[] = [
    ...
    postRoute(`${PET_API_PREFIX}/set-character`, (body) => {
      const character = body.character
      if (!isCharacterId(character)) return Promise.reject(new Error('invalid-character'))
      return service.setCharacter(character)
    }),
  ...
  const assetRoutes: WebRoute[] = ASSET_GROUPS.flatMap((group) =>
    ASSET_FILES.map((file): WebRoute => ({
      kind: 'exact',
      path: `${group.prefix}/${file.name}`,
      handler: (req: IncomingMessage, res: ServerResponse): Promise<void> | void => {
        ...
        return readFile(join(packageRoot, 'assets', group.assetDir, file.name)).then((body) => {
          res.writeHead(200, {
