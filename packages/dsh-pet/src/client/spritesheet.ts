/**
 * Pet spritesheet geometry and animation tracks.
 *
 ...
 *
 * Frame counts and per-frame durations are per-character track definitions.
 * The whale-girl atlas is produced by the hatch-pet pipeline and the
 * Phrolova atlas by `scripts/generate-pet-phrolova-placeholder.mjs`
 * (4 frames per row); calibrate the tables when real art lands.
...
/**
 * Whale-girl tracks. Durations are tuned for a soft, slow-healing feel
 * (roughly 2.5× the earlier fast draft — the pet should breathe, not race).
 */
const WHALE_TRACKS: Record<PetAnimation, TrackDef> = {
...
/**
 * Phrolova placeholder tracks (4 frames per row). Timing follows the
 * handoff guidance: idle/waiting breathe at 400-500ms, running rows move
 * at 225-250ms, jumping celebrates around 300-350ms and failed slows down.
 */
const PHROLOVA_TRACKS: Record<PetAnimation, TrackDef> = {
  idle: { frames: [0, 1, 2, 3], durations: [450, 450, 500, 450], loop: true },
  'running-right': { frames: [0, 1, 2, 3], durations: [225, 225, 225, 225], loop: true },
  'running-left': { frames: [0, 1, 2, 3], durations: [225, 225, 225, 225], loop: true },
  waving: { frames: [0, 1, 2, 3], durations: [350, 350, 350, 350], loop: true },
  jumping: { frames: [0, 1, 2, 3], durations: [280, 300, 320, 350], loop: false, fallback: 'idle' },
  failed: { frames: [0, 1, 2, 3], durations: [400, 450, 500, 600], loop: false, fallback: 'idle' },
  waiting: { frames: [0, 1, 2, 3], durations: [450, 450, 500, 450], loop: true },
  running: { frames: [0, 1, 2, 3], durations: [250, 250, 250, 250], loop: true },
  review: { frames: [0, 1, 2, 3], durations: [500, 500, 500, 500], loop: true },
}

/** Track tables per character. */
export const TRACKS_BY_CHARACTER: Record<CharacterId, Record<PetAnimation, TrackDef>> = {
  'whale-girl': WHALE_TRACKS,
  'phrolova': PHROLOVA_TRACKS,
}
