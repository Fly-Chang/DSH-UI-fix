...
export interface PetInjected {
  ...
  pet: () => void
  /** Feed the current character. */
  feed: () => void
  /** Switch the pet to the requested character. */
...
export function PetDockEntry(props: PetDockEntryProps): ReactElement {
    ...
    return (
      <span data-pet-dock data-testid="pet-dock">
        <WhalePet
          ...
          onPet={props.pet}
          onFeed={props.feed}
          onSwitchCharacter={props.switchCharacter}
          onHide={props.hide}
  ...
  return (
    ...
    >
      {props.t('pet.summon', { name: snapshot?.name ?? props.t('pet.state.loading') })}
    </button>
  )
}
