...
export function NewTaskModal({ controller, onClose }: { controller: BoardController; onClose: () => void }) {
  ...
  // Portaled to document.body so the fixed backdrop stays viewport-anchored
  // even when a skin frosts the board view with backdrop-filter (which would
  // otherwise become the containing block for this overlay).
  return createPortal(
