...
> 给 ui 插件做的最新改进款，修复了一些操作不便的地方。

本仓库是 [zhu1090093659/dsh-web-ui](https://github.com/zhu1090093659/dsh-web-ui) 的个人改进与留存版。基于上游的插件与皮肤集合，持续修复使用不便并新增功能。

## 与上游的主要差异

- **dsh-pet 双角色**：新增弗洛洛角色（亮/暗双形态跟随官方主题、面板一键换装、彼岸花投喂与专属等级/文案）；鲸鱼娘保留，亲密度与库存跨角色共享。
- **宠物兼容修复**：新版客户端遇到旧版 host 字段缺失时降级显示鲸鱼娘，不再白屏。
- **配套脚本**：`scripts/generate-pet-phrolova-placeholder.mjs`（占位图集生成）、`scripts/verify-pet-spritesheet.mjs`（图集几何校验）、`scripts/smoke-pet-phrolova.mjs`（重启后冒烟）。

## 留存与更新规则

- `origin`：`git@github.com:Fly-Chang/DSH-UI-fix.git`（SSH，本仓库）
- `upstream`：`git@github.com:zhu1090093659/dsh-web-ui.git`（SSH，上游）
- 每次改动后：本地提交 -> `git push origin main`；所有 push/fetch 一律走 SSH。
